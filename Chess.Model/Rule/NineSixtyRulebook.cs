...
namespace Chess.Model.Rule
{
    ...
    /// </summary>
    public class NineSixtyRulebook : IRulebook
    {
        /// <summary>
        ...
        /// <summary>
        /// Initializes a new instance of the <see cref="NineSixtyRulebook"/> class.
        /// </summary>
        public NineSixtyRulebook()
        {
        ...
        /// this is where I swaped the starting postions of the back row. The positions remain the same.
        public ChessGame CreateGame()
        {
            IEnumerable<PlacedPiece> makeBaseLine(int row, Color color)
            {
                ...
                yield return new PlacedPiece(new Position(row, 3), new Knight(color));
                yield return new PlacedPiece(new Position(row, 1), new Bishop(color));
                yield return new PlacedPiece(new Position(row, 7), new Queen(color));
                yield return new PlacedPiece(new Position(row, 2), new King(color));
                yield return new PlacedPiece(new Position(row, 4), new Bishop(color));
                yield return new PlacedPiece(new Position(row, 5), new Knight(color));
                yield return new PlacedPiece(new Position(row, 6), new Rook(color));
            }
